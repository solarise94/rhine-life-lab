...
MVP 只做 HTML 报告导出。Word / PDF 导出后置，避免过早处理模板、分页、字体和跨平台渲染问题。
...
```text
assistant_message / progress_note → Card 旁当前短气泡
...
Card 旁只保留一条当前气泡。ACP / executor 的流式输出应采用 active preview 模型：先创建一个当前气泡，然后用后续 `delta` / `snapshot` 更新同一个气泡内容，而不是把每个 token/chunk 渲染成新气泡。

实现上可以参考 OpenClaw Telegram preview streaming 的思路：可编辑通道用一个预览消息反复 edit，final 再复用或替换这个预览；本项目 UI 则对应为同一个 Card bubble 原地更新。

完整输出仍写入 run event stream / transcript；气泡只是当前进度摘要，不承担日志查看功能。长文本或分块输出进入详情/日志，不进入多个气泡。
