...
- off：普通 `Send` 图标；
- on idle：使用 `Sparkles`、`Bot` 或 `Workflow` 图标，保持静态；
- on running / thinking：切换为停止/打断态图标，使用运行色和呼吸动画；
- 按钮外圈显示细环；
- tooltip: `Auto mode 已开启`；
- 输入框附近显示小型 `AUTO` chip，可点击打开 `status / off / stop / once` 菜单。

详细 composer 状态机见 `docs/25_manager_auto_input_button_ui_plan.md`。
...
- switch send button style by composer state: normal, auto idle, auto running;
