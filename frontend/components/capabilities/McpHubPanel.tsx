...
import { useProjectMcpLibrary } from "@/lib/hooks";
...
export function McpHubPanel({ projectId, focusId }: McpHubPanelProps) {
  const sectionRef = useRef<HTMLElement>(null);
  const libraryQuery = useProjectMcpLibrary(projectId);
        ...
        <div className="settings-library-grid">
          <div className="settings-library-list">
            {libraryQuery.isLoading ? (
              <div className="settings-empty">正在加载 MCP 库…</div>
            ) : null}
            {!libraryQuery.isLoading && !filtered.length ? (
              <div className="settings-empty">没有匹配的 MCP</div>
            ) : null}
            {filtered.map((item) => (
              <button
                key={String(item.id)}
          ...
          <div className="settings-library-detail">
            ...
            {selected ? (
              ...
              <div className="settings-empty">在左侧列表中选择一个 MCP 查看详情</div>
            )}
          </div>
        </div>
      </div>
