...
from app.models.chat import ChatSessionMessage, ChatSessionMessageTimelineItem, ChatRequest
...
class ManagerWakeProcessor:
    ...
    def _process_project(self, project_id: str) -> None:
        ...
        try:
            ...
            response = self.manager_service.chat(project_id, request)
            self.chat_session_service.append_messages(
                project_id,
                owner_session_id,
                [self._manager_response_message(wake_event, response.message, response.thinking)],
                dedupe_ids=[f"wake_response_{wake_event.wake_id}"],
            )
