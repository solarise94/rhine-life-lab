...
class ManagerBlueprintTools:
    ...
    def plan_blueprint(self, project_id: str, payload: dict) -> dict:
        ...
        existing_planned_assets = self._planned_card_output_producers(snapshot["cards"])
        ...
        for step in normalized_steps:
            ...
            for asset_ref in step["input_assets"]:
                ...
                if asset_id in existing_assets and existing_assets[asset_id].status in {"valid", "candidate"}:
                    ...
                    continue
                producing_card_id = existing_planned_assets.get(asset_id)
                if producing_card_id:
                    asset_sufficiency.append(
                        {
                            "label": label,
                            "asset_id": asset_id,
                            "state": "planned_from_existing_card",
                            "producer_card_id": producing_card_id,
                        }
                    )
                    block_reasons.append(f"waiting for planned input {asset_id} from card {producing_card_id}")
                    continue
    ...
    @staticmethod
    def _planned_card_output_producers(cards: list[Card]) -> dict[str, str]:
        producers: dict[str, str] = {}
        for card in cards:
            for output in card.outputs:
                if output.asset_id:
                    producers.setdefault(output.asset_id, card.card_id)
        return producers
