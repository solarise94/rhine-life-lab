...
def list_project_data_directory_entries(
    ...
    return MountedDataDirectoryService(project_service).list_entries(
        project_id,
        path=path,
        kind=kind,
        cursor=cursor,
...
def register_data_directory_asset(
    ...
    return MountedDataDirectoryService(project_service).register_asset(project_id, path=request.path)
...
def export_asset_to_data_directory(
    ...
    return MountedDataDirectoryService(project_service, worker_service).export_asset(
        project_id,
        asset_id=asset_id,
